...
public class CodeGenMain {
    ...
    public static class ReadMysql {
        ...
        public DataBaseModel genarateMysqlDataBase(Properties p) {
            ...
            String author = p.getProperty("author","tuping");
            //--------------生成代码前修改如下配置 end----------------------------//
        ...
        private TableModel parseTableByShowCreate(Connection conn, String tablename, String tableNamePrefix, String packname) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    for (int i = 0; i < lines.length; i++) {
                        String line = lines[i];
                        //String regex = "\\s*`([^`]*)`\\s*(\\w+[^ ]*)\\s*(NOT\\s+NULL\\s*)?(DEFAULT\\s*([^ ]*|NULL|'0'|''|CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6))\\s*)?(\\s*AUTO_INCREMENT\\s*)?(COMMENT\\s*'([^']*)')?\\s*,\\s*";
                        ...
                        while (m.find()) {
                            try {
                                ...
                                column.setdBcolumnName(field);
                                field = StringUtil.getCamelCase(field, "_");
            ...
            if(table.getPkClass()==null) {
            	table.setPkClass("Long");
            }
            return table;
        }
